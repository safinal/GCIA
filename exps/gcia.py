...
class GCIA(Experiment):
    def __init__(self):
        super().__init__('GCIA')
        
    def create_balanced_dataloader_ll(self, balanced_dataset_path, sample_size, batch_size, feature_only, dataset, **kwargs):
        if feature_only:
            features = torch.load(os.path.join(balanced_dataset_path, "gcia_features.pt"))
            labels = torch.load(os.path.join(balanced_dataset_path, "gcia_labels.pt"))
